...
use Drupal\smplphotoalbum\Controller\GetID3\Module\Tag\ID3v1;
						...
						if (substr($atom_data, 2, 2) == "\x10\xB5") {
							...
							while ($atomoffset < strlen($atom_data)) {
								...
								switch ($boxsmalltype) {
									case "\x10\xB5":
										...
										break;
									default:
										$this->warning('Unknown QuickTime smallbox type: "'.preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $boxsmalltype).'" ('.trim(Utils::PrintHexBytes($boxsmalltype)).') at offset '.$baseoffset);
							...
							while ($atomoffset < strlen($atom_data)) {
								...
								switch ($boxtype) {
									...
									default:
										$this->warning('Unknown QuickTime box type: "'.preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $boxtype).'" ('.trim(Utils::PrintHexBytes($boxtype)).') at offset '.$baseoffset);
										$atom_structure['data'] = $atom_data;
...
//					if (true) {
	...
	/* due to lack of known documentation, this is a kludge implementation. If you know of documentation on how mdat is properly structed, please send it to info@getid3.org */
					...
					if (substr($atom_data, $mdat_offset, 4) == 'GPRO') {
						$GOPRO_chunk_length = Utils::LittleEndian2Int(substr($atom_data, $mdat_offset + 4, 4));
						$GOPRO_offset = 8;
					...
					while (($mdat_offset < (strlen($atom_data) - 8))
						&& ($chapter_string_length = Utils::BigEndian2Int(substr($atom_data, $mdat_offset, 2)))
				...
				case 'chpl': // CHaPter List
					...
					$chpl_version = Utils::BigEndian2Int(substr($atom_data, 4, 1)); // Expected to be 0
					$chpl_flags   = Utils::BigEndian2Int(substr($atom_data, 5, 3)); // Reserved, set to 0
					$chpl_count   = Utils::BigEndian2Int(substr($atom_data, 8, 1));
					$chpl_offset = 9;
					for ($i = 0; $i < $chpl_count; $i++) {
						...
						$info['quicktime']['chapters'][$i]['timestamp'] = Utils::BigEndian2Int(substr($atom_data, $chpl_offset, 8)) / 10000000; // timestamps are stored as 100-nanosecond units
						$chpl_offset += 8;
						$chpl_title_size = Utils::BigEndian2Int(substr($atom_data, $chpl_offset, 1));
						$chpl_offset += 1;
				...
				case 'rcif':
					...
					if (substr($atom_data, 0, 7) == "\x00\x00\x00\x00\x55\xC4".'{') {
						...
						$this->warning('Expecting 55 C4 7B at start of atom "'.$atomname.'", found '.Utils::PrintHexBytes(substr($atom_data, 4, 3)).' instead');
						$atom_structure['data'] = $atom_data;
					}
					break;
				...
				case 'cdsc': // timed metadata reference
					...
					$atom_structure['track_number'] = Utils::BigEndian2Int($atom_data);
					break;
