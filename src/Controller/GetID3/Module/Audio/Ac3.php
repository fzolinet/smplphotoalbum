...
class Ac3 extends Handler
{
	...
	public function Analyze() {
		...
		$this->AC3header['syncinfo']  =     Utils::BigEndian2Int(substr($tempAC3header, 0, 2));
		$this->AC3header['bsi']       =     Utils::BigEndian2Bin(substr($tempAC3header, 2));
		$thisfile_ac3_raw_bsi['bsid'] = (Utils::LittleEndian2Int(substr($tempAC3header, 5, 1)) & 0xF8) >> 3; // AC3 and E-AC3 put the "bsid" version identifier in the same place, but unfortnately the 4 bytes between the syncword and the version identifier are interpreted differently, so grab it here so the following code structure can make sense
		unset($tempAC3header);
		...
		if ($thisfile_ac3_raw_bsi['bsid'] <= 8) {
			...
			$thisfile_ac3_raw_bsi['crc1']       = Utils::Bin2Dec($this->readHeaderBSI(16));
