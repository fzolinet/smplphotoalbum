...
namespace Drupal\smplphotoalbum\Controller\GetID3\Module\Audio;
...
class Dsdiff extends Handler
{
	...
	public function Analyze() {
		...
		if (substr($DSDIFFheader, 0, 4) != 'FRM8') {
			$this->error('Expecting "FRM8" at offset '.$info['avdataoffset'].', found "'.Utils::PrintHexBytes(substr($DSDIFFheader, 0, 4)).'"');
			return false;
		}
		unset($DSDIFFheader);
		$this->fseek($info['avdataoffset']);
		...
		while (!$this->feof() && ($ChunkHeader = $this->fread(12))) {
			...
			if (!preg_match('#^[\\x21-\\x7E]+ *$#', $thisChunk['name'])) {
				...
				$this->error('Invalid chunk name "'.$thisChunk['name'].'" ('.Utils::PrintHexBytes($thisChunk['name']).') at offset '.$thisChunk['offset'].', aborting parsing');
			}
			$thisChunk['size'] = Utils::BigEndian2Int(substr($ChunkHeader, 4, 8));
			...
			switch ($thisChunk['name']) {
				case 'FRM8':
					...
					if ($thisChunk['form_type'] != 'DSD ') {
						$this->error('Expecting "DSD " at offset '.($this->ftell() - 4).', found "'.Utils::PrintHexBytes($thisChunk['form_type']).'", aborting parsing');
						break 2;
					}
				...
				case 'PROP': // PROPerty chunk
					...
					if ($thisChunk['prop_type'] != 'SND ') {
						$this->error('Expecting "SND " at offset '.($this->ftell() - 4).', found "'.Utils::PrintHexBytes($thisChunk['prop_type']).'", aborting parsing');
						break 2;
					}
					// do nothing further, prevent skipping subchunks
					break;
				...
				case 'FS  ': // sample rate chunk
					if ($thisChunk['size'] == 4) {
						$info['dsdiff']['sample_rate'] = Utils::BigEndian2Int($this->fread(4));
				...
				case 'CHNL': // CHaNneLs chunk
					$thisChunk['num_channels'] = Utils::BigEndian2Int($this->fread(2));
				...
				case 'CMPR': // CoMPRession type chunk
					...
					$humanReadableByteLength = Utils::BigEndian2Int($this->fread(1));
				...
				case 'ABSS': // ABSolute Start time chunk
					$ABSS = $this->fread(8);
					$info['dsdiff']['absolute_start_time']['hours']   = Utils::BigEndian2Int(substr($ABSS, 0, 2));
					$info['dsdiff']['absolute_start_time']['minutes'] = Utils::BigEndian2Int(substr($ABSS, 2, 1));
					$info['dsdiff']['absolute_start_time']['seconds'] = Utils::BigEndian2Int(substr($ABSS, 3, 1));
					$info['dsdiff']['absolute_start_time']['samples'] = Utils::BigEndian2Int(substr($ABSS, 4, 4));
					unset($ABSS);
					break;
				case 'LSCO': // LoudSpeaker COnfiguration chunk
					...
					$thisChunk['loundspeaker_config_id'] = Utils::BigEndian2Int($this->fread(2));
					break;
				case 'COMT': // COMmenTs chunk
					$thisChunk['num_comments'] = Utils::BigEndian2Int($this->fread(2));
					for ($i = 0; $i < $thisChunk['num_comments']; $i++) {
						$thisComment = array();
						$COMT = $this->fread(14);
						$thisComment['creation_year']   = Utils::BigEndian2Int(substr($COMT,  0, 2));
						$thisComment['creation_month']  = Utils::BigEndian2Int(substr($COMT,  2, 1));
						$thisComment['creation_day']    = Utils::BigEndian2Int(substr($COMT,  3, 1));
						$thisComment['creation_hour']   = Utils::BigEndian2Int(substr($COMT,  4, 1));
						$thisComment['creation_minute'] = Utils::BigEndian2Int(substr($COMT,  5, 1));
						$thisComment['comment_type_id'] = Utils::BigEndian2Int(substr($COMT,  6, 2));
						$thisComment['comment_ref_id']  = Utils::BigEndian2Int(substr($COMT,  8, 2));
						$thisComment['string_length']   = Utils::BigEndian2Int(substr($COMT, 10, 4));
				...
				case 'MARK': // MARKer chunk
					$MARK = $this->fread(22);
					$thisChunk['marker_hours']   = Utils::BigEndian2Int(substr($MARK,  0, 2));
					$thisChunk['marker_minutes'] = Utils::BigEndian2Int(substr($MARK,  2, 1));
					$thisChunk['marker_seconds'] = Utils::BigEndian2Int(substr($MARK,  3, 1));
					$thisChunk['marker_samples'] = Utils::BigEndian2Int(substr($MARK,  4, 4));
					$thisChunk['marker_offset']  = Utils::BigEndian2Int(substr($MARK,  8, 4));
					$thisChunk['marker_type_id'] = Utils::BigEndian2Int(substr($MARK, 12, 2));
					$thisChunk['marker_channel'] = Utils::BigEndian2Int(substr($MARK, 14, 2));
					$thisChunk['marker_flagraw'] = Utils::BigEndian2Int(substr($MARK, 16, 2));
					$thisChunk['string_length']  = Utils::BigEndian2Int(substr($MARK, 18, 4));
				...
				case 'DITI': // title chunk
					$thisChunk['string_length']  = Utils::BigEndian2Int($this->fread(4));
					$thisChunk['description'] = ($thisChunk['string_length'] ? $this->fread($thisChunk['string_length']) : '');
					if ($thisChunk['string_length'] % 2) {
