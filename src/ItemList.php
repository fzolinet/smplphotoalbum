...
use Drupal\smplphotoalbum\Item;
...
class ItemList {
	...
	function __construct(&$params) {
		...
		$tn = Lib::slash( $this->root . $this->path . $this->subfolder . self::TN);
		$ok = true;
		if (! is_dir ( $tn )) {
			$ok = mkdir ( $tn );
		}
		...
		//Thumbnails refresh
		if( Lib::Request('SmplThumbnails')  && $this->access ) {
			$this->RefreshFolder();
		}
		
		// Upload file enabled		
		if( $this->upload && Lib::Request("SmplUploadSubmit") && $this->access ){
			$this->Upload();
		}

		// filter enabled
		if ( isset ( $this->params['filter'] ) && $this->params['filter'] ) {
			$this->filter = Lib::Request('smpl_filter',"" );
		}
...
//---------------------------------------
	...
	function preSettings() {	
	  ...
	  $this->slide_checking = $this->params['slide_checking'];
		...
		// Paging
		$smpl_page = Lib::Request("smpl_page", "" );
		if( !empty( $smpl_page ) ){
	 ...
	 */
	function Upload(){		
		$name = Lib::Request("smpl_uname", "", "POST");
		$sub  = Lib::Request("smpl_usub" , "", "POST");
		$type = Lib::Request("smpl_utype", "", "POST");
		$link = Lib::Request("smpl_ulink", "", "POST");
		$ti   = Lib::Request("smpl_utime", "", "POST");		
		$importance = Lib::Request("smpl_uimportance", "", "POST");
   ...
   */
	...
	function MakeThumbnail($name, $source, $thumbnail, &$msg, $folder = false ) {
		...
		if ( $this->isimage ( $name ) ) {
			...
			elseif ( $this->isvideohtml5($name ) || $this->isvideo( $name ) )
	...
	function Render() {
		...
		if( $this->access ) {			
			...
			//IMGEdit teszt
			if( Lib::Request("imgedit") ){
				$id = Lib::Request("id",1);
			}
		} else {

			// No access, remove edit forms
			$str = str_replace ( 
				[	
					"{{ EditForm }}", 
					"{{ ImgEditForm }}", 
   ...
   */
	...
	function CacheClear(){				
		...
		if(function_exists('apcu_clear_cache')){
			apcu_clear_cache();
			\Drupal::messenger()->addMessage("Reset APCU cache");
		}
	}
